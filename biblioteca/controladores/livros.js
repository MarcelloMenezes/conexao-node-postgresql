...
const excluirLivro = async (req, res) => {
    ...
    try {
        ...
        const verficar = await conexao.query('select * from livros join emprestimos on livros.id = emprestimos.id_livro where livros.id = $1 AND emprestimos.emprestimo = $2', [id, 'pendente']);

        if (verficar.rowCount) {
            return res.status(400).json("O livro está pendente em nosso sistema")
        }
