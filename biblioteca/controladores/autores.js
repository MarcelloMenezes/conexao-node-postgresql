...
const excluirAutor = async (req, res) => {
    ...
    try {
        ...
        const verficar = await conexao.query('select * from autores join livros on livros.autor_id = autores.id where autores.id = $1', [id]);

        if (verficar.rowCount) {
            return res.status(400).json("Apague os livros antes de deletar o autor")
        }
